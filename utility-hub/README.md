...
Production-ready Flask-based system to manage cron automation plus utility modules for address validation and URL health monitoring.
...
- `address_validations`: address validation requests and normalized results
- `monitored_urls`: URL inventory for health monitoring
- `execution_history` and `execution_details`: URL monitoring run summaries and per-URL checks
...
### Import Existing monitor-system Data (One-Time)
...
- `POST /api/monitoring/urls`
- `GET /api/monitoring/urls`
- `PUT /api/monitoring/urls/<id>`
- `DELETE /api/monitoring/urls/<id>`
- `POST /api/monitoring/execute`
- `GET /api/monitoring/reports`
- `GET /api/monitoring/reports/<id>`
- `GET /api/monitoring/scheduler/status`
- `POST /api/monitoring/scheduler/run-now`

Utility Hub web pages now include:
